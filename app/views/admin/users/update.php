...
<div class="card p-4 bg-light">
    ...
    <div class="card-body">
        <form action="<?= ROOT ?>adminUser/edit/<?= $data['data']->id ?>" method="POST">
            <div class="form-group text-left mb-2">
                <label for="name">Usuario:</label>
                <input type="text" name="name" id="name" class="form-control" required
                    placeholder="Escribe el nombre completo" value="<?= $data['data']->name ?? '' ?>">
            </div>
            <div class="form-group text-left mb-2">
                <label for="email">Correo electrónico:</label>
                <input type="email" id="email" name="email" class="form-control" required
                    placeholder="Escriba el correo electrónico" value="<?= $data['data']->email ?? '' ?>">
            </div>
            <div class="form-group text-left mb-2">
                <label for="password1">Contraseña: (Dejar en blanco si no desea cambiarla)</label>
                <input type="password" name="password1" class="form-control" placeholder="Escribe una contraseña">
            </div>
            <div class="form-group text-left mb-2">
            ...
            <div class="form-group">
                ...
                <select name="status" id="status" class="form-control">
                    ...
                    <?php foreach ($data['status'] as $status): ?>
                        <option value="<?= $status->value ?>" <?= $status->value == $data['data']->status ? 'selected' : '' ?>>
                            <?= $status->description ?>
                        </option>
                    <?php endforeach; ?>
                </select>
