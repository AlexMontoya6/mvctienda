        ...
        <form action="<?= ROOT ?>adminProduct/delete/<?= $data['product']->id ?>" method="POST"
            ...
            <div class="form-control text-left">
                ...
                <select name="type" id="type" class="form-control" disabled>
                    ...
                    <?php foreach ($data['type'] as $type): ?>
                        <option value="<?= $type->value ?>" <?= isset($data['product']->type) && $data['product']->type == $type->value ? 'selected' : '' ?>>
                            <?= $type->description ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="form-group text-left mt-4">
                ...
                <input type="text" name="name" id="name" class="form-control" disabled
                    placeholder="Escribe el nombre del producto" value="<?= $data['product']->name ?? '' ?>">
            </div>
