        ...
        <div class="collapse navbar-collapse" id="menu">
            <?php if ($data['menu']): ?>
                ...
                <ul class="nav navbar-nav navbar-right mt-2 mt-lg-0">
                    <?php if (isset($_SESSION['cartTotal']) && $_SESSION['cartTotal'] > 0): ?>
                        <li class="nav-item">
                            <a href="<?= ROOT ?>cart" class="nav-link">Carrito:
                                <?= number_format($_SESSION['cartTotal'], 2) ?> &euro;
                            </a>
                        </li>
                    <?php endif; ?>
                    <li class="nav-item">
                        <form action="<?= ROOT ?>search/products" class="d-flex form-inline" method="POST">
                            <input type="text" name="search" id="search" class="form-control" size="20"
            ...
            <?php if (isset($data['admin']) && $data['admin']): ?>
                <ul class="navbar-nav mr-auto mt-2 mt-lg-0">
                    ...
                    <li class="nav-item">
                        <a href="<?= ROOT . 'AdminShop/logout' ?>" class="nav-link">Salir</a>
                    </li>
                </ul>
            <?php endif; ?>
        </div>
